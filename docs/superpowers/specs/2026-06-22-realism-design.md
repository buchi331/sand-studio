...
### 2. 砂の自然な堆積
- 安息角の調整（対角移動の条件を見直し、急すぎる柱が緩やかに崩れる）。
- 質感は描画側の粒ごと色ゆらぎで補強（②）。挙動側は崩れ方のみ調整。

### 3. 火の温度（データ整備）
- `life`(0..FIRE_LIFE) を「温度」として描画で使う。挙動側は減衰カーブの調整のみ。
- レンダラが読めるよう、`life` をグリッドビューに公開（②で `GridView` 拡張）。

### テスト（vitest 追加）
- 水は油より速く横に広がる（同条件Nステップ後、水の広がり幅 > 油の広がり幅）。
- 油は流れにくい（同条件で油の横移動量 < 水）。
